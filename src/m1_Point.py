...
class Point(object):
    ...
    def move_to(self,x,y):
        ...
        self.y = y
        self.move_mades += 1
        self.distance_traveled += ((self.o-x)**2 + (self.k-y)**2)**(1/2)
    ...
    def get_number_of_moves_made(self):
        return self.move_mades
    def get_distance_from(self,point):
        return ((point.x-self.x)**2 + (point.y-self.y)**2)**(1/2)
    def get_distance_from_start(self):
        return ((self.x-self.p)**2 + (self.y-self.n)**2)**(1/2)
    def get_distance_traveled(self):
        return self.distance_traveled
    def closer_to(self,p2,p3):
        self.a = self.get_distance_from(p2)
        self.b = self.get_distance_from(p3)
        if self.a <= self.b:
            return p2
        else:
            return p3
    def halfway_to(self,p2):
        self.t = (self.x + p2.x)/2
        self.v = (self.y + p2.y)/2
        return Point(self.t, self.v)
def run_test_init():
    """
